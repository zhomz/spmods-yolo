...
class AmmoStats implements IPostDBLoadMod   {
	...
	private CalculateArmorLevel(penetrationValue)    {
	...
	public postDBLoad(container: DependencyContainer): void {
		...
		for (const itemID in itemTables) {	                                                        // Iterate through all itemIDs
			if (itemTables[itemID]._props.ammoType === "bullet" || itemTables[itemID]._props.ammoType === "buckshot") {
				...
				let bulletArmorTier: number = this.CalculateArmorLevel(bulletPenetration)	    // Calculate what the best tier is for that penetration value
		...
		for (const localeID in locales) {	                                                    // Iterate through all language options
			...
			for (const key in ammoStatDict) {
				...
				if (this.config.MODE.toLowerCase() === "prepend") {	                                	// Checking which mode was selected in the config
